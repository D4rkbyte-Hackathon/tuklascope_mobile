...
class CardConcept(BaseModel):
    domain: str = Field(
        ...,
        description="The official K-12/CHED Domain (e.g., 'Automotive Engineering', 'Philippine History')",
    )
    skill: str = Field(
        ...,
        description="The specific educational topic (e.g., 'Combustion Mechanics', 'Cultural Integration')",
    )
...
class CardChallenge(BaseModel):
    ...
    options: list[str] = Field(..., description="Array of 3 to 4 possible answers.")
...
class LearningDeckRequest(BaseModel):
    object_name: str = Field(..., description="The object the user scanned")
    chosen_lens: str = Field(..., description="The academic strand they clicked")
    grade_level: str = Field(..., description="The user's academic stage")
    teaser_context: str = Field(
        ...,
        description="The exact text of the teaser door they chose, to ensure continuity.",
    )
